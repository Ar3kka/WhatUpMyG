...
var selectable_object : RigidBody3D
...
func _deselect():
	...
	selected_object.get_node(SELECTABLE_NODE).select.emit(false, true)
	var draggable_component : DraggableComponent = selected_object.get_node(DRAGGABLE_NODE)
...
func _process(_delta):
	...
	if detected_object != null:
		...
		var lookable_component : LookableComponent = detected_object.get_node(LOOKABLE_NODE)
		...
		if !dragging():
			var selectable_component : SelectableComponent = lookable_object.get_node(SELECTABLE_NODE)
			...
			lookable_object.get_node(LOOKABLE_NODE).look.emit(false)
	...
	if selectable_object == null && selected_object == null: return
	...
	if selectable_object:
		if Input.is_action_just_pressed("Select"):
			...
			selectable_object.get_node(SELECTABLE_NODE).select.emit(true, true)
			selected_object = selectable_object
		
		# Checking if the selectable object IS draggable
		draggable_component = selectable_object.get_node(DRAGGABLE_NODE)
	...
	var team_component : TeamComponent = draggable_object.get_node(TEAM_NODE)
